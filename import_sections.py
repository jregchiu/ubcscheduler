...
with open(s) as f:
    for line in f:
        ...
        if s['course'] not in courses:
            ...
            sec = Section(course=c, year=int(s['year']), session=s['session'], code=s['code'], status=s.get('status'), activity=s['activity'], term=s['term'], days=s['days'], start=s['start'], end=s['end'])
            courses[s['course']] = c
            db.session.add(sec)
        else:
            c = courses[s['course']]
